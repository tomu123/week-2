...
select c.* from "Customers" as c join "Orders" as o on o.customer_id = c.id join "Order Lines" as ol on o.id = ol.order_id WHERE ol.product_id = 16;
...
select DISTINCT p.* from "Products" as p join "Order Lines" as ol on ol.product_id = p.id;
-- Select the total amount of products a X customer bought between 2 dates
select c.*,SUM(ol.quantity) as "amount of products" from "Order Lines" as ol join "Orders" as o on o.id = ol.order_id join "Customers" as c on c.id = o.customer_id where (o.date BETWEEN '2022-09-27' AND '2022-11-29') AND c.id = 2 GROUP BY c.id;
